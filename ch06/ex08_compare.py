"""
파라미터 최적화 알고리즘 6가지 성능 비교
    - 성능 비교 지표 : 손실(loss), 정확도(accuracy)
"""
...
if __name__ == '__main__':
    ...
    for i in range(iterations):   # 2,000번 학습 반복
        # 학습데이터(X_train), 학습 레이블(Y_train)에서 미니 배치 크기만큼 랜덤하게 데이터를 선택
        batch_mask = np.random.choice(train_size, batch_size)   # 60,000개 중 128개의 임의의 숫자 선택 - 인덱스

        # 0 ~ 59,999 사이의 숫자들(train size) 중에서 128(batch size)개의 숫자를 임의로 선택
        # 학습에 사용할 미니 배치 데이터/레이블 선택
        X_batch = X_train[batch_mask]
        Y_batch = Y_train[batch_mask]

        # 선택된 학습 데이터/레이블을 사용해서 gradient 계산
        for key in optimizers:
            # 각각의 최적화 알고리즘에 대해서 같은 작업 반복
            gradients = neural_nets[key].gradient(X_batch, Y_batch)
            # 각각의 최적화 알고리즘의 파라미터 업데이트 기능 사용
            # -> 신경망이 가지고 있는 파라미터(W, b)와 위에서 계산한 gradient를 넘김김
